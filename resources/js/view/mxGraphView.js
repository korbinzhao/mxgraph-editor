...
mxGraphView.prototype.validate = function (cell) {
  ...
  const graphBounds = this.getBoundingBox(this.validateCellState(
...
mxGraphView.prototype.getBackgroundPageBounds = function () {
  ...
  const bounds = new mxRectangle(this.scale * this.translate.x, this.scale * this.translate.y,
...
mxGraphView.prototype.stateValidated = function (state) {
  const fg = (this.graph.getModel().isEdge(state.cell) && this.graph.keepEdgesInForeground)
...
mxGraphView.prototype.getRoutingCenterX = function (state) {
  const f = (state.style != null) ? parseFloat(state.style
...
mxGraphView.prototype.installListeners = function () {
  ...
  if (container != null) {
    ...
    if (mxClient.IS_TOUCH) {
      mxEvent.addListener(container, 'gesturestart', mxUtils.bind(this, (evt) => {
        graph.fireGestureEvent(evt);
        mxEvent.consume(evt);
      }));
			
      mxEvent.addListener(container, 'gesturechange', mxUtils.bind(this, (evt) => {
        graph.fireGestureEvent(evt);
        mxEvent.consume(evt);
      }));

      mxEvent.addListener(container, 'gestureend', mxUtils.bind(this, (evt) => {
        graph.fireGestureEvent(evt);
        mxEvent.consume(evt);
      }));
    }
